...
def SingleMorphFeatures(args):
    ...
    for contour, bbox in tqdm(zip(contours, bboxes), desc="Processing Contours", total=len(contours)):
...
def \
    ...
    aaah=pd.DataFrame(centroids)
    normal_data=aaah.iloc[normalIDs,:]
    ...
    return globalGraph, edge_info,normal_data
